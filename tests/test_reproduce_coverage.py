...
背景:旧的 `reproduce.sh` 自称 "full reproduction chain",但实测
...
执行,要么不写):
  1. `docs/REPRODUCIBILITY.md` 复现矩阵里的每条
     `python experiments/X.py` 命令,其脚本必须在 reproduce.sh 的
     runner 表中出现;
  2. runner 表里登记的所有产物路径必须实际存在于仓库;
  3. README(en)必须引用 reproduce.sh(防止再次零引用)。
"""
...
def test_matrix_commands_covered():
    ...
    missing = sorted(c for c in cmds
                     if c not in text)
    assert not missing, (
        f"reproduce.sh does not run these matrix experiments: "
        f"{missing}")
...
def test_readme_references_the_entry_point():
    """README(en)必须引用 reproduce.sh(防零引用回归)。"""
    readme = README.read_text(encoding="utf-8")
    assert "reproduce.sh" in readme, (
        "README does not reference reproduce.sh -- the reproduction "
