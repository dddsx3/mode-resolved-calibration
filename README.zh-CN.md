...
- **认证动态范围**：全部 masked 像素、全部 142 灯下，重标所有 48 个
  ...
  **Σ_φ 参数族敏感性(P-SIGMA-FAMILY)**:把单线参数化(logI/方向方差比被
  ...
  违反 −0.0028);**两项定律的典型精度存活**(中位 |dV| 0.0076)但最坏
  情况参数化特定(通道隔离极端处最大 0.464);**线性化半径依赖通道**
  (joint 与仅强度:1.0/1.5,冻结值逐位复现;仅方向在整个 [0.05, 8] 网格
  从未离开一阶标度;het=1.0 使半径缩小 5 倍)。族机制在退化锚点上对冻结
  产物逐位复现(264/264 通道分解行 + 逐点锚点)。**知情序有效性(E)参数化特定**:控制臂精确复现冻结子集
  (88/125 = 0.704);方向更重的腐蚀使预测序**更**可信(0.889),但逐灯
  异质性把它破坏到低于随机(0.353,逐格 Spearman 中位 −0.78)——冻结的
  0.687 是均匀 σ 形状的性质,不是管线的性质
