...
| C4 | ~~Mode-tail targeted intervention is significant~~ **RETRACTED** — v1.0 was a gauge-alignment asymmetry artifact; the corrected three-arm rerun shows **no detectable difference** between mode-targeted and scalar-targeted allocation (all distinctive CIs span 0) | see `results/mode_tail/allocation_mode_tail.json` (`erratum`, `aggregated.distinctiveness`) | `results/mode_tail/allocation_mode_tail.json` | `pytest tests/test_m2_m3_evidence.py::test_alloc2_structure` |
