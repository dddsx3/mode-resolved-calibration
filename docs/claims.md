...
| B9 | Decision quality: the information allocation beats random on the physical reconstruction endpoint, and the predicted ordering is valid in 88/88 cells | on the normal angular error (single residual pipeline: corruption + fixed-n-hat GLS + one-step calibrated_ps normal refit): dAUC vs random a_opt -4.04 deg [CI -6.06, -2.57] @ regime 10, -5.29 [-7.71, -3.55] @ 100; mode_aware -3.82 [-6.00, -2.46] @ 10, -5.12 [-7.87, -3.45] @ 100 (object-cluster bootstrap B=10000); Spearman(predicted J_A, realized normal error) positive in 88/88 object x level x regime cells (median +0.750, min +0.357). The gauge-aligned endpoints show the redistribution phenomenon instead (Spearman medians -0.375 dual / -0.518 albedo-MSE; dAUC_dual positive) — the decision-layer validity envelope: information predictions are directionally valid for physical error; weak-mode endpoints need the coverage caveat (C8's E(k) hump) Informed-only discrimination: the predicted ordering agrees with the realized ordering in 130/175 = 0.743 [CI 0.67, 0.81] of (cell, budget) pairs within the informed family (mode_aware vs a_opt); the 88/88 headline is against universe-random only (no active-set-restricted random arm in this grid) | `results/openillumination/decision_quality.json` | `pytest tests/test_decision_quality.py` |
